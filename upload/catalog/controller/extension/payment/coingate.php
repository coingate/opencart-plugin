...
class ControllerExtensionPaymentCoingate extends Controller {
  ...
  public function checkout() {
    ...
    $cg_order = \CoinGate\Merchant\Order::create(array(
      ...
      'price'             => number_format($order_info['total'] * $this->currency->getvalue($order_info['currency_code']), 2, '.', ''),
  ...
  private function setupCoingateClient() {
    \CoinGate\CoinGate::config(array(
      ...
      'app_id'      => $this->config->get('payment_coingate_app_id'),
      'api_key'     => $this->config->get('payment_coingate_api_key'),
      'api_secret'  => $this->config->get('payment_coingate_api_secret'),
